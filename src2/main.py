...
def clause_extraction():
    ...
    sd = [
        [('Человек', ('f', ('x',)))],  # Человек{Сократ}
        [('not', ('Человек', ('x',))), ('Смертен', ('x',))],  # ¬Человек{x} ∨ Смертен{x}
        [('not', ('Смертен', ('f', ('x',))))]  # ¬Смертен{Сократ}
    ]
    ...
    # P(f(a)) ∧ (¬P(x) ∨ Q(x)) → Q(f(a))
    # P(f(A)) - предикат P с аргументом f(A), где f - функция, A - константа
    func1 = [
            [('P', ('f', 'A'))],  # P(f(A))
            [('not', ('P', 'x')), ('Q', 'x')],  # ¬P(x) ∨ Q(x)
            [('not', ('Q', ('f', 'A')))]  # ¬Q(f(A))
    ...
    # P(f(x), g(y)) ∧ (¬P(a, b) ∨ Q(h(z))) ∧ ¬Q(h(c))
    func2 = [
        [('P', ('f', 'x'), ('g', 'y'))],  # P(f(x), g(y))
        [('not', ('P', 'a', 'b')), ('Q', ('h', 'z'))],  # ¬P(a, b) ∨ Q(h(z))
        [('not', ('Q', ('h', 'c')))]  # ¬Q(h(c))
    ]
...
if __name__ == "__main__":
    clauses1, clauses2 = clause_extraction()
    ...
    print("P(f(a)) ∧ (¬P(x) ∨ Q(x)) → Q(f(a))")
    prove(clauses1)

    print("-" * 30 + "\n")
    print("P(f(x), g(y)) ∧ (¬P(a, b) ∨ Q(h(z))) ∧ ¬Q(h(c))")
    prove(clauses2)
